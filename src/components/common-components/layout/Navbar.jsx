...
import React, { useState, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom'; // <-- make sure this is here
...
const Header = () => {
  ...
  const handleLogoClick = (e) => {
    ...
    if (location.pathname === '/') {
      // On LandingPage → scroll to hero
      scrollToSection('hero');
    } else {
      // On other pages → navigate back to LandingPage
      navigate('/');
    }
  };
  return (
    <header id="header" className={`header d-flex align-items-center fixed-top ${isScrolled ? 'scrolled' : ''}`}>
      <div className="container-fluid container-xl position-relative d-flex align-items-center">
        ...
        <a className="cta-btn" href="#about" onClick={goToCartPage} > {/* Shop Icon always visible */} <i className="bi bi-cart3"></i> {/* Text visible only on md and larger screens */} <span className="d-none d-md-inline" style={{ marginLeft: "5px" }}> Get Shop </span> </a>
      </div>
    </header>
  );
};
